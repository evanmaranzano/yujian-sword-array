...
### 9.8 v6d（2026-09-08 第四轮，用户真机反馈 + GitHub Pages 上线）

- **双拳八卦重做**（用户反馈"还是不像八卦"）：从八门径向剑臂改为经典卦符画法——中心 108 剑
  阴阳环 + 8 卦各 3 条直爻（阳爻 8 剑密排整条 0.75、阴爻两短条中缝 0.18），卦符紧凑 0.75×0.86、
  卦心半径 2.36、先天八卦圆序（TRIG 爻码表）绕心缓转。迭代 3 版才读得出"卦"：径向臂→弧爻
  （糊成同心环，圆周格式塔会把同半径弧连成环）→直条卦符（条 1.6 太长又连成八边形）→紧凑短条
  （0.75×0.86 + 卦间距 ~1.0）才成立。
- **剑指"指哪飞哪"**：剑群拆 204 剑云 + 96 背景万剑。挥剑指时 `launchCloud`——剑云保持队形整体
  射出（全部 FIRE 同速 30-36 u/s + ±0.05rad 散开角，`fscale` 数组保持剑形小尺度不被 FIRE 默认
  0.85 放大）、背景万剑同向跟随、飞出屏幕 RETURN 自动回手重聚剑形。onSwipe 在 BIG_SWORD
  分流到 launchCloud，其余阵型仍走 burst。
- **ROCK 手势文案**：HUD/手势表标注"食指+小指"（🤟）。
- **GitHub Pages 上线**：https://evanmaranzano.github.io/yujian-sword-array/ （根 index.html 跳转
...
### 9.10 v7（2026-09-10 用户需求：深度融合 + 999 剑 + 精度增强）

- **999 把飞剑**：CONFIG.swordCount 500→999（移动端/小屏 400，同大庚原版降级策略）。热循环
  全部去内存分配（模块级 _tgt/_des/_steer/_sep/_look 临时向量），999×60fps 无 GC 抖动——
  这同时是"实时控制效果不够"的帧稳定性修复。
- **恢复七大自研阵型（v6e 融合时丢失，全部退化为游龙）**：BAGUA（双拳）/PILLAR（点赞）/
  HEXAGRAM（六字诀）/RAIN（下压）/INFINITY（交叉）/EXPLODE（推开）/ENERGY_BALL（捧起）
  以"目标点+期望朝向"方式接入大庚 steering 循环（`_look` 临时向量传递朝向）。
  **关键坑**：①大庚分离力（间距<3 互推）会把密集爻线推散成横带——自研密集阵型必须禁用分离力
  （sepOn 只对 DRAGON/DAGENG/未跟手 LOTUS 开）；②原版冲刺 50+转向力 28/s 上限对静态目标产生
  ±10 级永久过冲振荡——自研阵型用"速度 18+转向力 4x+arriveR 6"专用参数；③按剑尺度数组 scl[]
  替换原版全局 userData.currentScale（八卦环细剑 0.5）；④自研阵型不挂 isTracking 门
  （手势丢失由 IDLE 发布链负责切回莲花，阵型不闪跳）。
- **八卦阵（保留项）**：183 剑阴阳环 + 8 卦×3 直爻×34 剑（阳爻整条 1.5、阴爻两段中缝 1.2）、
  卦心半径 6.5、整体绕心缓转、细剑 0.5。布局迭代：条太长会连成八边形轮廓（格式塔），
  紧凑卦符+卦间距>卦符尺寸才成立。
- **手势识别精度包（展厅亮光鲁棒）**：
  ① 21 点 landmark One-Euro 滤波（LandmarkSmoother，只作用于分类路径，位置/挥舞各走各的滤波）；
  ② 滑窗 5 帧多数票：单帧误判被历史票数压掉；
  ③ 非对称稳定计时：IDLE→手势 130ms 快进（手感），手势→IDLE 350ms 慢放（不闪跳），
     gestureStableMs 180 用于手势→手势；
  ④ 手指判定升级"关节角+距离比"双通道（伸直：角>150° 或比>1.16；弯曲：角<110° 或比<0.97，
     任一成立即算，互为冗余抗抖动抗旋转）；y 轴链式比较全部退役；
  ⑤ 亮光应对：启动时尝试锁手动曝光压补偿（getCapabilities 探测，不支持静默跳过）+
     每 0.5s 采样 16×12 缩略图平均亮度，>225 时 HUD 显示"⚠ 环境光过亮"警示；
  ⑥ minHandTrackingConfidence 0.7→0.6（过曝场保持追踪）。
